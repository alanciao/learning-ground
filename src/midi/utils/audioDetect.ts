...
const supports:AudioDetectSupports = {};
let pending = 0;

function canPlayThrough(src:string) {
  pending++;
...
export function audioDetect(onsuccess:Function) {
  ...
  if ((navigator as any).requestMIDIAccess) {
    ...
    if (isNative) {
      ...
      for (let i = 0; i < navigator.plugins.length; i++) {
        ...
        if (plugin.name.indexOf('Jazz-Plugin') !== -1) {
          supports.webmidi = true;
        }
      }
    }
  ...
  const vorbis = audio.canPlayType('audio/ogg; codecs="vorbis"');
  const canPlayVorbis = vorbis === 'probably' || vorbis === 'maybe';
  const mpeg = audio.canPlayType('audio/mpeg');
  const canPlayMPEG = mpeg === 'probably' || mpeg === 'maybe';
  // maybe nothing is supported
  if (!canPlayVorbis && !canPlayMPEG) {
    return onsuccess(supports);
  }
